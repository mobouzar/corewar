...
int get_size_arg(uint8_t flg, int nb_arg, int opcode, int *err);
int get_size_beyt_flag(t_process *p, uint8_t flg, int opcode);
...
uint32_t cpy_arena_to_var(t_process *p, int size, int cursor0, int opcode);
uint32_t return_data_of_arg(t_process *proc, uint8_t flg, int opcode, int cursor0);
